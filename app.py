...
if st.session_state['active_section'] == 'Network Analysis':
    selected_coins = st.multiselect("Select Cryptocurrencies (max 10)", list(coin_options.keys()), key_network_coins", max_options=10)
    if st.button("Analyze Network", key="network_btn"):
        if len(selected_coins) >= 2:
            with st.spinner("Analyzing network..."):
                try:
                    days = {"1 Day": 14, "7 Days": 14, "6 Months": 90, "1 Year": 90}
                    time_horizon = st.selectbox("Select Time Horizon for Correlation", ["1 Day", "7 Days", "6 Months", "1 Year"], key="network_horizon")
                    # Fetch historical data for selected coins
                    price_data = []
                    for coin_name in selected_coins:
                        coin_id = coin_options[coin_name]
                        data = fetch_historical_data(coin_id, days[time_horizon])
                        price_data.append(data['price'].values)
                    # Calculate correlation matrix
                    price_df = pd.DataFrame(np.array(price_data).T, columns=selected_coins)
                    ...
                    for i in range(len(selected_coins)):
                        for j in range(i + 1, len(selected_coins)):
                            ...
                            if abs(corr) > 0.5:  # Threshold for edge creation
                                G.add_edge(selected_coins[i], selected_coins[j], weight=corr)
                    # Visualize network
                    ...
                    fig = go.Figure(data=[edge_trace, node_trace],
                                    layout=go.Layout(title="Price Correlation Network", showlegend=False,
                                                     margin=dict(b=0, l=0, r=0, t=50)))
                    st.plotly_chart(fig, use_container_width=True)
                    st.write("Edges represent correlations > 0.5")
                    st.success("Network analysis completed!")
                except Exception as e:
                    st.error(f"Error: {str(e)}")
                    logger.error(f"Network analysis error: {e}")
        else:
            st.error("Select at least two cryptocurrencies")
...
if st.session_state['active_section'] == 'Blockchain Monitor':
    ...
    if st.button("Monitor", key="blockchain_btn"):
        with st.spinner("Fetching blockchain data..."):
            try:
                coin_id = coin_options[coin_name]
                blockchain_data = fetch_blockchain_data(coin_id)
                st.write("**Blockchain Statistics**")
                st.write(f"Transactions (24h): {blockchain_data['transactions_24h'].iloc[0]}")
                st.write(f"Mean Fee (USD): ${blockchain_data['mean_fee_usd'].iloc[0]:.2f}")
                st.write(f"Active Addresses (24h): {blockchain_data['active_addresses_24h'].iloc[0]}")
                st.write(f"Difficulty: {blockchain_data['difficulty'].iloc[0]:.2f}")
                st.success("Blockchain monitoring completed!")
            except Exception as e:
                st.error(f"Error: {str(e)}")
...
if st.session_state['active_section'] == 'DeFi Insights':
    if st.button("Fetch DeFi Data", key="defi_btn"):
        with st.spinner("Fetching DeFi data..."):
            try:
                defi_data = fetch_defi_data()
                st.write("**DeFi Protocol Insights**")
                st.dataframe(defi_data)
                fig = px.bar(defi_data, x='Protocol', y='TVL', color='Chain', title="Total Value Locked by Protocol")
                st.plotly_chart(fig, use_container_width=True)
                st.success("DeFi insights retrieved!")
            except Exception as e:
                st.error(f"Error: {str(e)}")
...
if st.session_state['active_section'] == 'Social Sentiment':
    ...
    days = st.selectbox("Select Time Period", [1, 7, 14, 30], key="sentiment_days")
    if st.button("Analyze Sentiment", key="sentiment_btn"):
        with st.spinner("Analyzing sentiment..."):
            try:
                sentiment_data = get_social_sentiment(coin_name, days)
                st.write(f"**Sentiment Score for {coin_name}**: {sentiment_data['Sentiment'].mean():.2f}")
                fig = px.line(sentiment_data, x='ds', y='Sentiment', title=f"{coin_name} Sentiment Trend")
                st.plotly_chart(fig, use_container_width=True)
...
if st.session_state['active_section'] == 'Trading Alerts':
    ...
    sentiment_threshold = st.number_input("Sentiment Threshold", min_value=0.1, max_value=1.0, value=0.2, step=0.1)
    if st.button("Check Alerts", key="alerts_btn"):
        with st.spinner("Checking alerts..."):
            try:
                ...
                st.success("Alerts check completed!")
            except Exception as e:
                st.error(f"Error: {str(e)}")
    ...
    if st.button("Simulate Deployment", key="decentralized_btn"):
        with st.spinner("Simulating decentralized deployment..."):
            try:
                ...
                st.write("**Decentralized Deployment Simulation**")
                st.write(result)
...
if st.session_state['active_section'] == 'Explainable AI':
    coin_name = st.selectbox("Select Cryptocurrency", list(coin_options.keys()), key="explainable_coin")
    time_horizon = st.selectbox("Select Time Horizon", ["1 Day", "7 Days"], key="explainable_horizon")
    if st.button("Explain Prediction", key="explainable_btn"):
        with st.spinner("Generating explanation..."):
            try:
                ...
                if shap_data:
                    ...
                    shap.summary_plot(shap_values, pd.DataFrame(X_val_scaled, columns=features), show=False)
                    plt.tight_layout()
                    st.pyplot(plt)
                    st.success("Explanation generated!")
                else:
                    st.warning("SHAP explanations are only available for short-term predictions (1 Day, 7 Days)")
            except Exception as e:
                st.error(f"Error: {str(e)}")
                logger.error(f"Explainable AI error: {e}")
        ...
        with st.spinner("Rebalancing portfolio..."):
            try:
                ...
                if weights:
                    ...
                    fig = px.pie(names=list(weights.keys()), values=list(weights.values()), title="Rebalanced Portfolio")
                    st.plotly_chart(fig, use_container_width=True)
...
if st.session_state['active_section'] == 'Transaction Analysis':
    coin_name = st.selectbox("Select Cryptocurrency", list(coin_options.keys()), key="transaction_coin")
    if st.button("Analyze Transactions", key="transaction_btn"):
        with st.spinner("Analyzing transactions..."):
            try:
                coin_id = coin_options[coin_name]
                nodes, edges = fetch_transaction_graph(coin_id)
                features = np.random.rand(len(nodes), 10)  # Simulated node features
                ...
                if gnn_model:
                    ...
                    st.write("GNN Output (Node Scores):")
                    st.write(out.numpy().flatten()[:5])  # Show first 5 for brevity
                    G = nx.Graph()
                    for edge in edges:
                    ...
                    node_trace = go.Scatter(
                        ...
                        marker=dict(size=20, color='lightblue'), hoverinfo='text')
                    fig = go.Figure(data=[edge_trace, node_trace],
                                    layout=go.Layout(title="Transaction Network", showlegend=False,
                                                     margin=dict(b=0, l=0, r=0, t=50)))
                    st.plotly_chart(fig, use_container_width=True)
                    st.success("Transaction analysis completed!")
                else:
                    st.error("GNN model training failed")
            except Exception as e:
                st.error(f"Error: {str(e)}")
...
if st.session_state['active_section'] == 'Market Scenarios':
    coin_name = st.selectbox("Select Cryptocurrency", list(coin_options.keys()), key="scenarios_coin")
    n_scenarios = st.number_input("Number of Scenarios", min_value=1, max_value=10, value=5, step=1)
    if st.button("Generate Scenarios", key="scenarios_btn"):
        with st.spinner("Generating market scenarios..."):
            try:
                ...
                if vae_model and scaler:
                    scenarios = generate_scenarios(vae_model, scaler, n_scenarios)
                    st.write("**Market Scenarios**")
                    scenario_df = pd.DataFrame(scenarios, columns=['Price', 'Volume', '24h Change (%)'])
                    st.dataframe(scenario_df)
                    fig = px.line(scenario_df, y='Price', title=f"{coin_name} Simulated Price Scenarios")
                    st.plotly_chart(fig, use_container_width=True)
                    st.success("Market scenarios generated!")
                else:
                    st.error("VAE model training failed")
            except Exception as e:
                st.error(f"Error: {str(e)}")
